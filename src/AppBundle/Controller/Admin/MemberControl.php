...
class MemberControl extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $users = $em->getRepository('AppBundle:Member')->findAll();

        return $this->xRender('Admin/Lid/index.html.twig', array(
            'users' => $users,
        ));
    }
    ...
    public function newAction(Request $request, UserPasswordEncoderInterface $passwordEncoder){
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            if($bestaande_user==null && $bestaande_email == null)
            {
                ...
                $this->addFlash("ok", "Member geregistreerd");
                return $this->redirectToRoute('admin_lid_show', array('id' => $member->getId()));
            }
        ...
        return $this->xRender('Admin/Lid/new.html.twig', array(
            'user' => $member,
    ...
    public function showAction(Member $user)
    {
        ...
        return $this->xRender('Admin/Lid/show.html.twig', array(
            'user' => $user,
     ...
     *
     * @Route("/{id}/edit", name="admin_lid_edit")
     * @Method({"GET", "POST"})
     */
    public function editAction(Request $request, Member $user)
    {
        $deleteForm = $this->createDeleteForm($user);
        $editForm = $this->createForm('AppBundle\Form\MemberType', $user);
        ...
        return $this->xRender('Admin/Lid/edit.html.twig', array(
            'Lid' => $user,
     ...
     *
     * @Route("/{id}", name="admin_lid_delete")
     * @Method("DELETE")
     */
    public function deleteAction(Request $request, Member $user)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash("ok", "Member verwijderd");
        }
     ...
     *
     * @param User $user The Lid entity
     *
    ...
    private function createDeleteForm(User $user)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('admin_lid_delete', array('id' => $user->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
